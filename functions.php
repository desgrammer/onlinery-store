<?php
/**
 * Onlinery Store functions and definitions
 *
...
if ( ! defined( 'ONLINERY_STORE_VERSION' ) ) {
	// Replace the version number of the theme on each release.
	define( 'ONLINERY_STORE_VERSION', '0.0.1' );
}

if ( ! function_exists( 'onlinery_store_setup' ) ) :
	/**
	 ...
	 */
	function onlinery_store_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on Onlinery Store, use a find and replace
		 * to change 'onlinery-store' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'onlinery-store', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'onlinery-store' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'onlinery_store_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'onlinery_store_setup' );
 ...
 */
function onlinery_store_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'onlinery_store_content_width', 640 );
}
add_action( 'after_setup_theme', 'onlinery_store_content_width', 0 );
 ...
 */
function onlinery_store_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'onlinery-store' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'onlinery-store' ),
...
add_action( 'widgets_init', 'onlinery_store_widgets_init' );
...
add_action( 'wp_enqueue_scripts', 'onlinery_store_scripts' );
 ...
 */
if ( ! function_exists( 'onlinery_store_customize_comment_fields' ) ) {
	function onlinery_store_customize_comment_fields( $fields ) {
...
add_filter( 'comment_form_fields', 'onlinery_store_customize_comment_fields' );